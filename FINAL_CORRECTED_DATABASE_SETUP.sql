-- ================================================================
-- TRONIC PLATFORM - COMPLETE DATABASE SETUP (CORRECTED VERSION)
-- Fixed all issues: extensions, schema qualification, RLS policies
-- ================================================================

-- STEP 1: ENABLE REQUIRED EXTENSIONS (CRITICAL - MUST BE FIRST)
-- This is needed for gen_random_uuid() function
CREATE EXTENSION IF NOT EXISTS "pgcrypto";

-- STEP 2: SET PROPER SEARCH PATH
-- Ensures we create objects in public schema
SET search_path = public, pg_catalog;

-- STEP 3: CLEAN SLATE - DROP ALL EXISTING OBJECTS
-- Drop in reverse dependency order with schema qualification
...
-- Drop indexes (with schema qualification)
DROP INDEX IF EXISTS idx_system_metrics_created_at ON public.system_metrics;
DROP INDEX IF EXISTS idx_system_metrics_name ON public.system_metrics;
DROP INDEX IF EXISTS idx_user_activity_created_at ON public.user_activity;
DROP INDEX IF EXISTS idx_user_activity_resource_type ON public.user_activity;
DROP INDEX IF EXISTS idx_user_activity_action ON public.user_activity;
DROP INDEX IF EXISTS idx_user_activity_user_id ON public.user_activity;
DROP INDEX IF EXISTS idx_command_logs_created_at ON public.command_logs;
DROP INDEX IF EXISTS idx_command_logs_status ON public.command_logs;
DROP INDEX IF EXISTS idx_command_logs_user_id ON public.command_logs;
DROP INDEX IF EXISTS idx_messages_message_type ON public.messages;
DROP INDEX IF EXISTS idx_messages_timestamp ON public.messages;
DROP INDEX IF EXISTS idx_messages_user_id ON public.messages;
DROP INDEX IF EXISTS idx_users_role ON public.users;
DROP INDEX IF EXISTS idx_users_email ON public.users;
DROP INDEX IF EXISTS idx_sessions_expires_at ON public.sessions;
DROP INDEX IF EXISTS idx_sessions_user_id ON public.sessions;
DROP INDEX IF EXISTS idx_sessions_token ON public.sessions;
...
-- IMPORTANT: Using TO service_role USING (true) instead of auth.role()
